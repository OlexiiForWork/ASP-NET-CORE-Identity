...
namespace Base.CustomPolicyProvider
{
    ...
    public static class DinamicAuthorizationFactory
    {
    ...
    /*При IAuthorizationPolicyProvider-мы должны переоприделить два фундоментальных метода 
     * GetDefaultPolicyAsync() и  GetFallbackPolicyAsync()
     * которые уже определены в DefaultAuthorizationPolicyProvider 
     * */
    ...
    //IAuthorizationPolicyProvider
    {
        ...
        public override Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            foreach (var customPolicy in DynamicPolicies.Get())
            {
                ...
                {
                    var policy = DinamicAuthorizationFactory.Create(policyName);
